...
class Users{
	...
	public function register($username,$password,$email,$email2,$email3,$fullname,$Telp,$level,$locked)
	{	$time 		= time();
		$ip 		= $_SERVER['REMOTE_ADDR'];
		$email_code = sha1($username + microtime());
		$password   = sha1($password);
	 	$query 	= $this->db->prepare("INSERT INTO `users` (`username`,`level`, `password`, `fullname`, `email`,`email2`,`email3`, `Telp`,`ip`, `time`, `email_code`, `confirmed`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
	 	$query->bindValue(1, $username);
		$query->bindValue(2, $level);
		$query->bindValue(3, $password);
		$query->bindValue(4, $fullname);
		$query->bindValue(5, $email);
		$query->bindValue(6, $email2);
		$query->bindValue(7, $email3);
		$query->bindValue(8, $Telp);
		$query->bindValue(9, $ip);
		$query->bindValue(10, $time);
		$query->bindValue(11, $email_code);
	...
	public function update($id,$username,$password,$email,$email2,$email3,$fullname,$Telp,$level,$locked)
	{	$time 		= time();
		$ip 		= $_SERVER['REMOTE_ADDR'];
		$email_code = sha1($username + microtime());
		$password   = sha1($password);
	 	$query 	= $this->db->prepare("UPDATE `users` SET `level` = ? , `password` = ? , `fullname` = ? , `email` = ? ,`email2` = ? ,`email3` = ? ,`Telp` = ? ,`ip` = ? , `time` = ? , `email_code` = ? ,`confirmed` = ? WHERE `id` = ?");
		$query->bindValue(1, $level);
		$query->bindValue(2, $password);
		$query->bindValue(3, $fullname);
		$query->bindValue(4, $email);
		$query->bindValue(5, $email2);
		$query->bindValue(6, $email3);
		$query->bindValue(7, $Telp);
		$query->bindValue(8, $ip);
		$query->bindValue(9, $time);
		$query->bindValue(10, $email_code);
		$query->bindValue(11, $locked);
	...
	{	$query = $this->db->prepare("SELECT * FROM `users` ORDER BY `time` DESC");
		...
		public function get_users_assign()
	{	$query = $this->db->prepare("SELECT * FROM `users` where `id` not in('27','28','32') ORDER BY `time` DESC");//hanya ditampilkan team terkait saja.
		try{
			$query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
