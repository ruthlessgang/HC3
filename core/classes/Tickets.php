...
class Tickets{
	...
	public function add_ticket($ticketnumber,$sla,$cat,$emp_id,$idcustomer,$entity,$emp_name,$emp_posisi,$emp_telp,$reportedby,$reporteddate,$telp,$email,$problemsummary,$problemdetail,$ticketstatus,$assignee,$documentedby)
	{	$current  = time();
		$querystring = "INSERT INTO `tickets` (`ticketnumber`,`sla`,`cat`,`emp_id`,`idcustomer`,`entity`,`emp_name`,`emp_posisi`,`emp_telp`,`reportedby`,`reporteddate`,`telp`,`email`,`problemsummary`,`problemdetail`,`ticketstatus`,`assignee`,`documentedby`,`documenteddate`) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$query 	= $this->db->prepare($querystring);
	 	$query->bindValue(1, $ticketnumber);
		$query->bindValue(2, $sla);
		$query->bindValue(3, $emp_id);
		$query->bindValue(4, $cat);
		$query->bindValue(5, $idcustomer);
		$query->bindValue(6, $entity);
		$query->bindValue(7, $emp_name);
		$query->bindValue(8, $emp_posisi);
		$query->bindValue(9, $emp_telp);
		$query->bindValue(10, $reportedby);
		$query->bindValue(11, $reporteddate);
		$query->bindValue(12, $telp);
		$query->bindValue(13, $email);
		$query->bindValue(14, $problemsummary);
		$query->bindValue(15, $problemdetail);
		$query->bindValue(16, $ticketstatus);
		$query->bindValue(17, $assignee);
		$query->bindValue(18, $documentedby);
		$query->bindValue(19, $current);
	 	try{
			$query->execute();
	 	}catch(PDOException $e){
			die($e->getMessage());
