...
<?php
	...
	$mysqli = new mysqli("10.50.234.33", "root", "P@ssw0rd", "hc3");
...
<table bgcolor="#000000" cellspacing="1" cellpadding="3">	
	...
	<tr bgcolor="#FFFFFF">
		...
		<td align="center">
			<a href="javascript:getSelected('<?=$row->no_int?>','<?=$row->emp_id?>','<?=$row->emp_name?>','<?=$row->idcustomer?>','<?=$row->entity?>','<?=$row->emp_posisi?>','<?=$row->emp_telp?>')">
				<?=$row->emp_id?>
			</a>
		</td>
		<td><?=$row->emp_name?></td>
